...
int ModApiMainMenu::l_native_update_formspec(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	NativeModApiMainMenu::n_update_formspec(engine, formspec);
	return 0;
}
/******************************************************************************/
...
int ModApiMainMenu::l_native_set_formspec_prepend(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	std::string formspec(luaL_checkstring(L, 1));
	NativeModApiMainMenu::n_set_formspec_prepend(engine, formspec);
...
int ModApiMainMenu::l_native_start(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	MainMenuData newData;

	newData.selected_world = getIntegerData(L, "selected_world", valid) - 1;
	newData.simple_singleplayer_mode = getBoolData(L, "singleplayer", valid);
	newData.do_reconnect = getBoolData(L, "do_reconnect", valid);
	if (!newData.do_reconnect) {
		newData.name = getTextData(L, "playername");
		newData.password = getTextData(L, "password");
		newData.address = getTextData(L, "address");
		newData.port = getTextData(L, "port");
	}
	newData.serverdescription = getTextData(L, "serverdescription");
	newData.servername = getTextData(L, "servername");

	NativeModApiMainMenu::n_start(engine, newData);
...
int ModApiMainMenu::l_native_close(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);

	NativeModApiMainMenu::n_close(engine);
	return 0;
}
...
int ModApiMainMenu::l_native_set_background(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	bool retval = NativeModApiMainMenu::n_set_background(engine, backgroundlevel, texturename, tile_image, minsize);
	lua_pushboolean(L, retval);
	return 1;
}
/******************************************************************************/
...
int ModApiMainMenu::l_native_set_clouds(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	NativeModApiMainMenu::n_set_clouds(engine, value);
...
int ModApiMainMenu::l_native_get_textlist_index(lua_State *L)
{
	// get_table_index accepts both tables and textlists
	return l_native_get_table_index(L);
}
...
int ModApiMainMenu::l_native_get_table_index(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);

	std::string tablename(luaL_checkstring(L, 1));
	s32 selection = NativeModApiMainMenu::n_get_table_index(engine, tablename);
...
int ModApiMainMenu::l_native_get_worlds(lua_State *L)
{
	std::vector<WorldSpec> worlds = NativeModApiMainMenu::n_get_worlds();
...
int ModApiMainMenu::l_native_get_games(lua_State *L)
{
	std::vector<SubgameSpec> games = NativeModApiMainMenu::n_get_games();
...
int ModApiMainMenu::l_native_get_content_info(lua_State *L)
{
	std::string path = luaL_checkstring(L, 1);

	ContentSpec spec = NativeModApiMainMenu::n_get_content_info(path);
...
int ModApiMainMenu::l_native_show_keys_menu(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);

	NativeModApiMainMenu::n_show_keys_menu(engine);
	return 0;
}
...
int ModApiMainMenu::l_native_create_world(lua_State *L)
{
	const char *name = luaL_checkstring(L, 1);
	int gameidx = luaL_checkinteger(L, 2) - 1;

	std::unique_ptr<std::string> pushed_string = NativeModApiMainMenu::n_create_world(name, gameidx);
	if (!pushed_string)
		lua_pushnil(L);
	else
	{
		lua_pushstring(L, (*pushed_string).c_str());
	}
	return 1;
}
...
int ModApiMainMenu::l_native_delete_world(lua_State *L)
{
	int world_id = luaL_checkinteger(L, 1) - 1;
	std::unique_ptr<std::string> pushed_string = NativeModApiMainMenu::n_delete_world(world_id);
	if (pushed_string)
	{
		lua_pushstring(L, (*pushed_string).c_str());
		return 1;
...
int ModApiMainMenu::l_native_set_topleft_text(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	NativeModApiMainMenu::n_set_topleft_text(engine, text);
	return 0;
}
...
int ModApiMainMenu::l_native_get_mapgen_names(lua_State *L)
{
	std::vector<const char *> names;
	bool include_hidden = lua_isboolean(L, 1) && readParam<bool>(L, 1);
	NativeModApiMainMenu::n_get_mapgen_names(names, include_hidden);
...
int ModApiMainMenu::l_native_get_user_path(lua_State *L)
{
	std::string path = NativeModApiMainMenu::n_get_user_path();
	lua_pushstring(L, path.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_get_modpath(lua_State *L)
{
	std::string modpath = NativeModApiMainMenu::n_get_modpath();
	lua_pushstring(L, modpath.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_get_clientmodpath(lua_State *L)
{
	std::string modpath = NativeModApiMainMenu::n_get_clientmodpath();
	lua_pushstring(L, modpath.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_get_gamepath(lua_State *L)
{
	std::string gamepath = NativeModApiMainMenu::n_get_gamepath();
	lua_pushstring(L, gamepath.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_get_texturepath(lua_State *L)
{
	std::string gamepath = NativeModApiMainMenu::n_get_texturepath();
	lua_pushstring(L, gamepath.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_get_texturepath_share(lua_State *L)
{
	std::string gamepath = NativeModApiMainMenu::n_get_texturepath_share();
	lua_pushstring(L, gamepath.c_str());
	return 1;
}
/******************************************************************************/
...
int ModApiMainMenu::l_native_get_cache_path(lua_State *L)
{
	std::string cache_path = NativeModApiMainMenu::n_get_cache_path();
	lua_pushstring(L, cache_path.c_str());
	return 1;
}
/******************************************************************************/
...
int ModApiMainMenu::l_native_get_temp_path(lua_State *L)
{
	std::string temp_path = NativeModApiMainMenu::n_get_temp_path();
	lua_pushstring(L, temp_path.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_create_dir(lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);

	bool createdDir = NativeModApiMainMenu::n_create_dir(path);

	lua_pushboolean(L, createdDir);
	return 1;
}
...
int ModApiMainMenu::l_native_delete_dir(lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);

	bool deletedDir = NativeModApiMainMenu::n_delete_dir(path);

	lua_pushboolean(L, deletedDir);
	return 1;
}
...
int ModApiMainMenu::l_native_copy_dir(lua_State *L)
{
	const char *source = luaL_checkstring(L, 1);
	const char *destination = luaL_checkstring(L, 2);
	...
	bool copiedDir = NativeModApiMainMenu::n_copy_dir(source, destination, keep_source);

	lua_pushboolean(L, copiedDir);
	return 1;
}
...
int ModApiMainMenu::l_native_is_dir(lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);
	bool isDir = NativeModApiMainMenu::n_is_dir(path);
...
int ModApiMainMenu::l_native_extract_zip(lua_State *L)
{
	const char *zipfile = luaL_checkstring(L, 1);
	const char *destination = luaL_checkstring(L, 2);
	
	bool status = NativeModApiMainMenu::n_extract_zip(zipfile, destination);
...
int ModApiMainMenu::l_native_get_mainmenu_path(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	std::string mainmenuPath = NativeModApiMainMenu::n_get_mainmenu_path(engine);
	lua_pushstring(L, mainmenuPath.c_str());
	return 1;
}
...
int ModApiMainMenu::l_native_may_modify_path(lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);
	bool res = NativeModApiMainMenu::n_may_modify_path(path);
...
int ModApiMainMenu::l_native_show_path_select_dialog(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	sanity_check(engine != NULL);
	...
	NativeModApiMainMenu::n_show_path_select_dialog(engine, formname, title, is_file_select);
	return 0;
}
...
int ModApiMainMenu::l_native_download_file(lua_State *L)
{
	const char *url = luaL_checkstring(L, 1);
	const char *target = luaL_checkstring(L, 2);

	bool res = NativeModApiMainMenu::n_download_file(url, target);
	lua_pushboolean(L, res);
	return 1;
}
...
int ModApiMainMenu::l_native_get_video_drivers(lua_State *L)
{
	std::vector<irr::video::E_DRIVER_TYPE> drivers = NativeModApiMainMenu::n_get_video_drivers();
...
int ModApiMainMenu::l_native_get_video_modes(lua_State *L)
{
	std::vector<core::vector3d<u32>> videomodes = NativeModApiMainMenu::n_get_video_modes();
...
int ModApiMainMenu::l_native_gettext(lua_State *L)
{
	std::string text = NativeModApiMainMenu::n_gettext(luaL_checkstring(L, 1));
	lua_pushstring(L, text.c_str());
...
int ModApiMainMenu::l_native_get_screen_info(lua_State *L)
{
	NativeModApiMainMenu::ScreenInfo screenInfo = NativeModApiMainMenu::n_get_screen_info();

	lua_newtable(L);
	int top = lua_gettop(L);
	lua_pushstring(L, "density");
	lua_pushnumber(L, screenInfo.density);
	lua_settable(L, top);

	lua_pushstring(L, "display_width");
	lua_pushnumber(L, screenInfo.display_width);
	lua_settable(L, top);

	lua_pushstring(L, "display_height");
	lua_pushnumber(L, screenInfo.display_height);
	lua_settable(L, top);

	lua_pushstring(L, "window_width");
	lua_pushnumber(L, screenInfo.window_width);
	lua_settable(L, top);

	lua_pushstring(L, "window_height");
	lua_pushnumber(L, screenInfo.window_height);
	lua_settable(L, top);
	return 1;
}
...
int ModApiMainMenu::l_native_get_min_supp_proto(lua_State *L)
{
	int minSuppProto = NativeModApiMainMenu::n_get_min_supp_proto();
	lua_pushinteger(L, minSuppProto);
	return 1;
}

int ModApiMainMenu::l_native_get_max_supp_proto(lua_State *L)
{
	int maxSuppProto = NativeModApiMainMenu::n_get_max_supp_proto();
	lua_pushinteger(L, maxSuppProto);
	return 1;
}
...
int ModApiMainMenu::l_native_open_url(lua_State *L)
{
	std::string url = luaL_checkstring(L, 1);
	lua_pushboolean(L, NativeModApiMainMenu::n_open_url(url));
	return 1;
...
int ModApiMainMenu::l_native_open_dir(lua_State *L)
{
	std::string path = luaL_checkstring(L, 1);
	lua_pushboolean(L, NativeModApiMainMenu::n_open_dir(path));
	return 1;
}
...
int ModApiMainMenu::l_native_do_async_callback(lua_State *L)
{
	GUIEngine *engine = getGuiEngine(L);
	...
	int res = NativeModApiMainMenu::n_do_async_callback(engine, serialized_func_raw,
			serialized_param_raw, func_length, param_length);
...
void ModApiMainMenu::Initialize(lua_State *L, int top)
{
	...
	/*NATIVE CALLS*/
	API_FCT(native_update_formspec);
	API_FCT(native_set_formspec_prepend);
	API_FCT(native_set_clouds);
	API_FCT(native_get_textlist_index);
	API_FCT(native_get_table_index);
	API_FCT(native_get_worlds);
	API_FCT(native_get_games);
	API_FCT(native_get_content_info);
	API_FCT(native_start);
	API_FCT(native_close);
	API_FCT(native_show_keys_menu);
	API_FCT(native_create_world);
	API_FCT(native_delete_world);
	API_FCT(native_set_background);
	API_FCT(native_set_topleft_text);
	API_FCT(native_get_mapgen_names);
	API_FCT(native_get_user_path);
	API_FCT(native_get_modpath);
	API_FCT(native_get_clientmodpath);
	API_FCT(native_get_gamepath);
	API_FCT(native_get_texturepath);
	API_FCT(native_get_texturepath_share);
	API_FCT(native_get_cache_path);
	API_FCT(native_get_temp_path);
	API_FCT(native_create_dir);
	API_FCT(native_delete_dir);
	API_FCT(native_copy_dir);
	API_FCT(native_is_dir);
	API_FCT(native_extract_zip);
	API_FCT(native_may_modify_path);
	API_FCT(native_get_mainmenu_path);
	API_FCT(native_show_path_select_dialog);
	API_FCT(native_download_file);
	API_FCT(native_gettext);
	API_FCT(native_get_video_drivers);
	API_FCT(native_get_video_modes);
	API_FCT(native_get_screen_info);
	API_FCT(native_get_min_supp_proto);
	API_FCT(native_get_max_supp_proto);
	API_FCT(native_open_url);
	API_FCT(native_open_dir);
	API_FCT(native_do_async_callback);
}
