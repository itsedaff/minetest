...
#include "settings.h"
#include "log.h"
#include "native_api/native_mapgen.h"
...
int ModApiMapgen::l_native_get_biome_id(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	const u32 id = NativeModApiMapgen::n_get_biome_id(bmgr, biome_str);
	if (id == -1)
		return 0;
...
int ModApiMapgen::l_native_get_biome_name(lua_State *L)
{
	...
	std::string name = NativeModApiMapgen::n_get_biome_name(bmgr, biome_id);
...
int ModApiMapgen::l_native_get_heat(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	if (!NativeModApiMapgen::can_get_heat(settingsmgr, bmgr))
		return 0;

	float heat = NativeModApiMapgen::n_get_heat(settingsmgr, bmgr, pos);
...
int ModApiMapgen::l_native_get_humidity(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	if (!NativeModApiMapgen::can_get_humidity(settingsmgr, bmgr))
		return 0;

	float humidity = NativeModApiMapgen::n_get_humidity(settingsmgr, bmgr, pos);
	lua_pushnumber(L, humidity);
...
int ModApiMapgen::l_native_get_biome_data(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	MapSettingsManager *settingsmgr =
			getServer(L)->getEmergeManager()->map_settings_mgr;
	const BiomeManager *bmgr = getServer(L)->getEmergeManager()->getBiomeManager();
	if (!NativeModApiMapgen::can_get_biome_data(settingsmgr, bmgr, pos))
		return 0;

	NativeModApiMapgen::BiomeData b =
			NativeModApiMapgen::n_get_biome_data(settingsmgr, bmgr, pos);
	...
	lua_pushnumber(L, b.heat);
	lua_setfield(L, -2, "heat");

	lua_pushnumber(L, b.humidity);
	lua_setfield(L, -2, "humidity");
...
int ModApiMapgen::l_native_get_mapgen_object(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;

	const char *mgobjstr = lua_tostring(L, 1);

	enum MapgenObject mgobj = NativeModApiMapgen::n_get_mapgen_object(mgobjstr);
...
int ModApiMapgen::l_native_get_spawn_level(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	EmergeManager *emerge = getServer(L)->getEmergeManager();
	int spawn_level = NativeModApiMapgen::n_get_spawn_level(emerge, x, z);
	// Unsuitable spawn point
	if (spawn_level == MAX_MAP_GENERATION_LIMIT)
		return 0;
...
int ModApiMapgen::l_native_get_mapgen_params(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	MapSettingsManager *settingsmgr =
			getServer(L)->getEmergeManager()->map_settings_mgr;
	NativeModApiMapgen::MapgenParams mp = NativeModApiMapgen::n_get_mapgen_params(settingsmgr);

	lua_newtable(L);

	lua_pushstring(L, mp.mg_name.c_str());
	lua_setfield(L, -2, "mgname");

	std::istringstream ss(mp.seed);
	u64 seedint;
	ss >> seedint;
	lua_pushinteger(L, seedint);
	lua_setfield(L, -2, "seed");

	lua_pushinteger(L, stoi(mp.water_level, -32768, 32767));
	lua_setfield(L, -2, "water_level");

	lua_pushinteger(L, stoi(mp.chunk_size, -32768, 32767));
	lua_setfield(L, -2, "chunksize");

	lua_pushstring(L, mp.flags.c_str());
	lua_setfield(L, -2, "flags");
...
int ModApiMapgen::l_native_set_mapgen_params(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
...
int ModApiMapgen::l_native_get_mapgen_setting(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	bool res = NativeModApiMapgen::n_get_mapgen_setting(settingsmgr, name, setting);
	if (!res)
		return 0;
...
int ModApiMapgen::l_native_get_mapgen_setting_noiseparams(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	bool res = NativeModApiMapgen::n_get_mapgen_setting_noiseparams(settingsmgr, name, np);
	if (!res)
		return 0;
...
int ModApiMapgen::l_native_set_mapgen_setting(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeModApiMapgen::n_set_mapgen_setting(settingsmgr, name, value, override_meta);
...
int ModApiMapgen::l_native_set_mapgen_setting_noiseparams(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeModApiMapgen::n_set_mapgen_setting_noiseparams(settingsmgr, name, np, override_meta);
...
int ModApiMapgen::l_native_set_noiseparams(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeModApiMapgen::n_set_noiseparams(name, set_default, np);
...
int ModApiMapgen::l_native_get_noiseparams(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NoiseParams np;
	bool res = NativeModApiMapgen::n_get_noiseparams(name, np);
	if (!res)
		return 0;
...
int ModApiMapgen::l_native_set_gen_notify(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;

	u32 flags = 0, flagmask = 0;
	EmergeManager *emerge = getServer(L)->getEmergeManager();
	bool change_notify = read_flags(L, 1, flagdesc_gennotify, &flags, &flagmask);
	...
	if (lua_istable(L, 2)) {
		...
		while (lua_next(L, 2)) {
			if (lua_isnumber(L, -1))
				deco_ids.push_back(lua_tonumber(L, -1));	
			lua_pop(L, 1);
		}
	}

	NativeModApiMapgen::n_set_gen_notify(emerge, change_notify, flags, flagmask, deco_ids);
	return 0;
}
...
int ModApiMapgen::l_native_get_gen_notify(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	push_flags_string(L, flagdesc_gennotify, emerge->gen_notify_on,
			emerge->gen_notify_on);
	std::vector<u32> deco_ids = NativeModApiMapgen::n_get_gen_notify(emerge);

	lua_newtable(L);
	int i = 1;
	for (u32 deco_id : deco_ids) {
		lua_pushnumber(L, deco_id);
		lua_rawseti(L, -2, i++);
	}
	return 2;
}
...
int ModApiMapgen::l_native_get_decoration_id(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	int deco_id;
	bool res = NativeModApiMapgen::n_get_decoration_id(deco_str, dmgr, deco_id);
	if (!res)
		return 0;

	lua_pushinteger(L, deco_id);
	return 1;
}
...
int ModApiMapgen::l_native_register_biome(lua_State *L)
{
	...
	bool res = NativeModApiMapgen::n_register_biome(bmgr, biome, handle);
	if (!res)
		return 0;
...
int ModApiMapgen::l_native_register_decoration(lua_State *L)
{
	...
	ObjDefHandle handle = NativeModApiMapgen::n_register_decoration(ndef, decomgr, deco);
	if (handle == OBJDEF_INVALID_HANDLE)
		return 0;
	lua_pushinteger(L, handle);
	return 1;
...
int ModApiMapgen::l_native_register_ore(lua_State *L)
{
	...
	if (read_noiseparams(L, -1, &ore->np)) {
		...
		log_deprecated(L, "register_ore: ore type requires 'noise_params' but it "
				  "is not specified, falling back to defaults");
	}
	lua_pop(L, 1);
...
int ModApiMapgen::l_native_register_schematic(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	ObjDefHandle handle = NativeModApiMapgen::n_register_schematic(schemmgr, schem);
	lua_pushinteger(L, handle);
...
int ModApiMapgen::l_native_clear_registered_biomes(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;

	BiomeManager *bmgr = getServer(L)->getEmergeManager()->getWritableBiomeManager();
	NativeModApiMapgen::n_clear_registered_biomes(bmgr);
	return 0;
}
...
int ModApiMapgen::l_native_clear_registered_decorations(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;

	DecorationManager *dmgr =
			getServer(L)->getEmergeManager()->getWritableDecorationManager();
	NativeModApiMapgen::n_clear_registered_decorations(dmgr);
	return 0;
}
...
int ModApiMapgen::l_native_clear_registered_ores(lua_State *L)
{
	...
	NativeModApiMapgen::n_clear_registered_ores(omgr);
	return 0;
}
...
int ModApiMapgen::l_native_clear_registered_schematics(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;

	SchematicManager *smgr = getServer(L)->getEmergeManager()->getWritableSchematicManager();
	NativeModApiMapgen::n_clear_registered_schematics(smgr);
	return 0;
}
...
int ModApiMapgen::l_native_generate_ores(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeModApiMapgen::n_generate_ores(mg, oremgr, pmin, pmax);
	return 0;
}
...
int ModApiMapgen::l_native_generate_decorations(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeModApiMapgen::n_generate_decorations(mg, decomgr, pmin, pmax);
	return 0;
}
