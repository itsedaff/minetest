...
int ObjectRef::l_get_local_animation(lua_State *L)
{
	...
	int statusCode = NativeObjectRef::n_get_local_animation(player, frames, frame_speed);
...
int ObjectRef::l_native_get_local_animation(lua_State *L)
{
	...
	NativeObjectRef::n_get_local_animation(getplayer(ref), frames, frame_speed);
...
int ObjectRef::l_native_set_eye_offset(lua_State *L)
{
	...
	Server *server = getServer(L);
	int statusCode = NativeObjectRef::n_set_eye_offset(server, player, offset_first, offset_third);
	if (statusCode != 0)
		lua_pushboolean(L, true);
	return statusCode;
}
...
int ObjectRef::l_native_get_eye_offset(lua_State *L)
{
	...
	v3f *eye_offsets = NativeObjectRef::n_get_eye_offset(player);

	push_v3f(L, eye_offsets[0]);
	push_v3f(L, eye_offsets[1]);
...
int ObjectRef::l_native_send_mapblock(lua_State *L)
{
	...
	session_t peer_id = player->getPeerId();
	Server *server = getServer(L);
	bool r = NativeObjectRef::n_send_mapblock(server, peer_id, pos);
...
int ObjectRef::l_native_set_attach(lua_State *L)
{
	...
	int statusCode = NativeObjectRef::n_get_attach(sao, parent_id, bone, position, rotation, force_visible);
	if (parent_id) {
		ServerActiveObject *old_parent = env->getActiveObject(parent_id);
		old_parent->removeAttachmentChild(sao->getId());
	}
	...
	NativeObjectRef::n_set_attach(parent, sao, bone, position, rotation, force_visible);
	return 0;
}
...
int ObjectRef::l_native_get_children(lua_State *L)
{
	...
	const std::unique_ptr<std::unordered_set<int>> child_ids = NativeObjectRef::n_get_children(sao);
	if (!child_ids)
		return 0;
	int i = 0;
...
int ObjectRef::l_native_get_properties(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	ObjectProperties *prop = NativeObjectRef::n_get_properties(getobject(ref));
	if (!prop)
		return 0;
	push_object_properties(L, prop);
	return 1;
...
int ObjectRef::l_native_is_player(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	bool isPlayer = NativeObjectRef::n_is_player(getobject(ref));
...
int ObjectRef::l_native_set_nametag_attributes(lua_State *L)
{
	...
	if (!lua_isnil(L, -1)) {
		if (lua_toboolean(L, -1)) {
			if (read_color(L, -1, &n.bgcolor))
				n.has_bgcolor = true;
		}
	}
	lua_pop(L, 1);

	n.nametag = getstringfield_default(L, 2, "text", "");

	int status = NativeObjectRef::n_set_nametag_attributes(sao, n);
	if (status)
		lua_pushboolean(L, true);
	return status;
}
...
int ObjectRef::l_native_get_nametag_attributes(lua_State *L)
{
	...
	NativeObjectRef::NametagAttributes n = NativeObjectRef::n_get_nametag_attributes(sao);
	lua_newtable(L);

	push_ARGB8(L, n.color);
	lua_setfield(L, -2, "color");

	if (prop->nametag_bgcolor) {
		push_ARGB8(L, n.bgcolor);
		lua_setfield(L, -2, "bgcolor");
	} else {
		lua_pushboolean(L, false);
		lua_setfield(L, -2, "bgcolor");
	...
	lua_pushstring(L, n.nametag.c_str());
	lua_setfield(L, -2, "text");
...
int ObjectRef::l_native_set_velocity(lua_State *L)
{
	...
	NativeObjectRef::n_set_velocity(sao, vel);
	return 0;
}
...
int ObjectRef::l_native_add_velocity(lua_State *L)
{
	...
	NativeObjectRef::n_add_velocity(server, sao, vel);
...
int ObjectRef::l_native_get_velocity(lua_State *L)
{
	...
	v3f velocity = NativeObjectRef::n_get_velocity(sao);

	if (sao->getType() == ACTIVEOBJECT_TYPE_LUAENTITY) {
		pushFloatPos(L, velocity);
		return 1;
	} else if (sao->getType() == ACTIVEOBJECT_TYPE_PLAYER) {
...
int ObjectRef::l_native_set_acceleration(lua_State *L)
{
	...
	NativeObjectRef::n_set_acceleration(entitysao, acceleration);
	return 0;
}
...
int ObjectRef::l_native_set_rotation(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	NativeObjectRef::n_set_rotation(entitysao, rotation);
...
int ObjectRef::l_native_get_rotation(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	v3f rotation = NativeObjectRef::n_get_rotation(entitysao) * core::DEGTORAD;
...
int ObjectRef::l_native_set_yaw(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	NativeObjectRef::n_set_yaw(entitysao, yaw);
	return 0;
}
...
int ObjectRef::l_native_get_yaw(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	float yaw = NativeObjectRef::n_get_yaw(entitysao) * core::DEGTORAD;
...
int ObjectRef::l_native_set_texture_mod(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	NativeObjectRef::n_set_texture_mod(entitysao, mod);
...
int ObjectRef::l_native_get_texture_mod(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	std::string mod = NativeObjectRef::n_get_texture_mod(entitysao);
...
int ObjectRef::l_native_set_sprite(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	...
	NativeObjectRef::n_set_sprite(entitysao, start_frame, num_frames, framelength, select_x_by_camera);
	return 0;
}
...
int ObjectRef::l_native_get_entity_name(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
...
int ObjectRef::l_native_get_luaentity(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	LuaEntitySAO *entitysao = getluaobject(ref);
	int entityId = NativeObjectRef::n_get_luaentity(entitysao);
	if (entityId == -1)
		return 0;

	luaentity_get(L, entityId);
	return 1;
}
...
int ObjectRef::l_native_get_player_name(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	lua_pushstring(L, NativeObjectRef::n_get_player_name(player).c_str());
	return 1;
}
...
int ObjectRef::l_native_get_look_dir(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	v3f v = NativeObjectRef::n_get_look_dir(playersao);
	push_v3f(L, v);
	return 1;
}
...
int ObjectRef::l_native_get_look_pitch(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	lua_pushnumber(L, NativeObjectRef::n_get_look_pitch(playersao));
	return 1;
}
...
int ObjectRef::l_native_get_look_yaw(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	lua_pushnumber(L, NativeObjectRef::n_get_look_yaw(playersao));
	return 1;
}
...
int ObjectRef::l_native_get_look_vertical(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	lua_pushnumber(L, NativeObjectRef::n_get_look_vertical(playersao));
	return 1;
}
...
int ObjectRef::l_native_get_look_horizontal(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	lua_pushnumber(L, NativeObjectRef::n_get_look_horizontal(playersao));
	return 1;
}
...
int ObjectRef::l_native_set_look_vertical(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	NativeObjectRef::n_set_look_vertical(playersao, pitch);
...
int ObjectRef::l_native_set_look_horizontal(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	NativeObjectRef::n_set_look_horizontal(playersao, yaw);
...
int ObjectRef::l_native_set_look_pitch(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeObjectRef::n_set_look_pitch(playersao, pitch);
	return 1;
}
...
int ObjectRef::l_native_set_look_yaw(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	...
	NativeObjectRef::n_set_look_yaw(playersao, yaw);
	return 1;
}
...
int ObjectRef::l_native_set_fov(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	float transition_time = lua_isnumber(L, 4)
						...
						: 0.0f;
	PlayerFovSpec fov = {degrees, is_multiplier, transition_time};
	Server *server = getServer(L);
	
	NativeObjectRef::n_set_fov(player, server, fov);
	return 0;
}
...
int ObjectRef::l_native_get_fov(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	PlayerFovSpec fov_spec = NativeObjectRef::n_get_fov(player);
...
int ObjectRef::l_native_set_breath(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	NativeObjectRef::n_set_player_breath(playersao, breath);
...
int ObjectRef::l_native_get_breath(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	...
	u16 breath = NativeObjectRef::n_get_breath(playersao);
...
int ObjectRef::l_native_set_attribute(lua_State *L)
{
	...
	std::string attr = luaL_checkstring(L, 2);
	bool removeString = lua_isnil(L, 3);
	if (removeString) 
	{
		NativeObjectRef::n_set_attribute(playersao, attr, removeString);
	}
	else
	{
		std::string value = luaL_checkstring(L, 3);
		NativeObjectRef::n_set_attribute(playersao, attr, removeString, value);
	}
	return 1;
}
...
int ObjectRef::l_native_get_attribute(lua_State *L)
{
	...
	std::string value;
	if (NativeObjectRef::n_get_attribute(playersao, attr, value)) {
		lua_pushstring(L, value.c_str());
		return 1;
	}
...
int ObjectRef::l_get_meta(lua_State *L)
{
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	Metadata *m = NativeObjectRef::n_get_meta(playersao);

	if (m == nullptr)
		return 0;

	PlayerMetaRef::create(L, m);
	delete m;
	return 1;
}

int ObjectRef::l_native_get_meta(lua_State *L)
{
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	if (playersao == nullptr)
	...
	Metadata *meta = NativeObjectRef::n_get_meta(playersao);
	PlayerMetaRef::create(L, meta);
	return 1;
}
...
int ObjectRef::l_native_set_inventory_formspec(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	player->inventory_formspec = formspec;
	Server *server = getServer(L);
	NativeObjectRef::n_set_inventory_formspec(player, server, formspec);
	lua_pushboolean(L, true);
	return 1;
}
...
int ObjectRef::l_native_get_inventory_formspec(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	std::string formspec = NativeObjectRef::n_get_inventory_formspec(player);
...
int ObjectRef::l_native_set_formspec_prepend(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	NativeObjectRef::n_set_formspec_prepend(player, server, formspec);
	lua_pushboolean(L, true);
	return 1;
}
...
int ObjectRef::l_native_get_formspec_prepend(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	std::string formspec = NativeObjectRef::n_get_formspec_prepend(player);
...
int ObjectRef::l_native_get_player_control(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	PlayerControl *pc = NativeObjectRef::n_get_player_control(player);
	...
	const PlayerControl &control = *pc;
	lua_newtable(L);
	lua_pushboolean(L, control.up);
	lua_setfield(L, -2, "up");
	lua_pushboolean(L, control.down);
...
int ObjectRef::l_native_get_player_control_bits(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	u32 *ctrlBits = NativeObjectRef::n_get_player_control_bits(player);
...
int ObjectRef::l_native_set_physics_override(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	if (playersao == nullptr)
		return 0;
	NativeObjectRef::PhysicsOverride p;

	if (lua_istable(L, 2)) {
		bool modified = false;
		modified |= getfloatfield(
				L, 2, "speed", p.speed);
		modified |= getfloatfield(
				L, 2, "jump", p.jump);
		modified |= getfloatfield(
				L, 2, "gravity", p.gravity);
		modified |= getboolfield(
				L, 2, "sneak", p.sneak);
		modified |= getboolfield(L, 2, "sneak_glitch",
				p.sneak_glitch);
		modified |= getboolfield(
				L, 2, "new_move", p.new_move);
		if (modified)
			p.override_sent = false;
	} else {
		// old, non-table format
		// TODO: Remove this code after version 5.4.0
		log_deprecated(L,
				"Deprecated use of set_physics_override(num, num, num)");
		if (!lua_isnil(L, 2)) {
			p.speed = lua_tonumber(L, 2);
			p.override_sent = false;
		}
		if (!lua_isnil(L, 3)) {
			p.jump = lua_tonumber(L, 3);
			p.override_sent = false;
		}
		if (!lua_isnil(L, 4)) {
			p.gravity = lua_tonumber(L, 4);
			p.override_sent = false;
		}
	}
	NativeObjectRef::n_set_physics_override(playersao, p);
	return 0;
}
...
int ObjectRef::l_native_get_physics_override(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	PlayerSAO *playersao = getplayersao(ref);
	if (playersao == nullptr)
		return 0;
	NativeObjectRef::PhysicsOverride p = NativeObjectRef::n_get_physics_override(playersao);
	lua_newtable(L);
	lua_pushnumber(L, p.speed);
	lua_setfield(L, -2, "speed");
	lua_pushnumber(L, p.jump);
	lua_setfield(L, -2, "jump");
	lua_pushnumber(L, p.gravity);
	lua_setfield(L, -2, "gravity");
	lua_pushboolean(L, p.sneak);
	lua_setfield(L, -2, "sneak");
	lua_pushboolean(L, p.sneak_glitch);
	lua_setfield(L, -2, "sneak_glitch");
	lua_pushboolean(L, p.new_move);
	lua_setfield(L, -2, "new_move");
	return 1;
}
...
// hud_add(self, hud)
int ObjectRef::l_native_hud_add(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	u32 id = NativeObjectRef::n_hud_add(server, player, elem);
	if (id == U32_MAX) {
		return 0;
	}
...
int ObjectRef::l_native_hud_remove(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	if (!NativeObjectRef::n_hud_remove(server, player, id));
		return 0;
...
int ObjectRef::l_native_hud_change(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	NativeObjectRef::n_hud_change(server, player, id, stat, value);
...
int ObjectRef::l_native_hud_get(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	HudElement *elem = NativeObjectRef::n_hud_get(player, id);
	if (elem == nullptr)
		return 0;
...
int ObjectRef::l_native_hud_set_flags(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	if (!NativeObjectRef::n_hud_set_flags(server, player, flags, mask))
		return 0;
...
int ObjectRef::l_native_hud_get_flags(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	u32 flags = NativeObjectRef::n_hud_get_flags(player);
	lua_newtable(L);
	lua_pushboolean(L, flags & HUD_FLAG_HOTBAR_VISIBLE);
	lua_setfield(L, -2, "hotbar");
	lua_pushboolean(L, flags & HUD_FLAG_HEALTHBAR_VISIBLE);
	lua_setfield(L, -2, "healthbar");
	lua_pushboolean(L, flags & HUD_FLAG_CROSSHAIR_VISIBLE);
	lua_setfield(L, -2, "crosshair");
	lua_pushboolean(L, flags & HUD_FLAG_WIELDITEM_VISIBLE);
	lua_setfield(L, -2, "wielditem");
	lua_pushboolean(L, flags & HUD_FLAG_BREATHBAR_VISIBLE);
	lua_setfield(L, -2, "breathbar");
	lua_pushboolean(L, flags & HUD_FLAG_MINIMAP_VISIBLE);
	lua_setfield(L, -2, "minimap");
	lua_pushboolean(L, flags & HUD_FLAG_MINIMAP_RADAR_VISIBLE);
	lua_setfield(L, -2, "minimap_radar");
	return 1;
}
...
int ObjectRef::l_native_hud_set_hotbar_itemcount(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	s32 hotbar_itemcount = luaL_checkint(L, 2);
	Server *server = getServer(L);
	if (!NativeObjectRef::n_hud_set_hotbar_itemcount(server, player, hotbar_itemcount));
		return 0;
...
int ObjectRef::l_native_hud_get_hotbar_itemcount(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	s32 itemcount = NativeObjectRef::n_hud_get_hotbar_itemcount(player);
	if (itemcount == -1)
		return 0;
...
int ObjectRef::l_native_hud_set_hotbar_image(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	int status = NativeObjectRef::n_hud_set_hotbar_image(server, player, name);
...
int ObjectRef::l_native_hud_get_hotbar_image(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	std::unique_ptr<std::string> name = NativeObjectRef::n_hud_get_hotbar_image(player);
...
int ObjectRef::l_native_hud_set_hotbar_selected_image(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	int status = NativeObjectRef::n_hud_set_hotbar_selected_image(server, player, name);
	return status;
}
...
int ObjectRef::l_native_hud_get_hotbar_selected_image(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	std::unique_ptr<std::string> name = NativeObjectRef::n_hud_get_hotbar_selected_image(player);
...
//finish later
int ObjectRef::l_native_set_sky(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	std::tuple<SunParams, MoonParams, StarParams> celestial_params;

	bool deprecated_form = false;
	bool is_colorspec = is_color_table(L, 2);
	...
	if (lua_istable(L, 2) && !is_colorspec) {
		...
		*/
		...
		log_deprecated(L, "Deprecated call to set_sky, please check lua_api.txt");
		// Fix sun, moon and stars showing when classic textured skyboxes are used
		SunParams sun_params = NativeObjectRef::n_get_sun(player);
		MoonParams moon_params = NativeObjectRef::n_get_moon(player);
		StarParams star_params = NativeObjectRef::n_get_stars(player);
		...
		Server *server = getServer(L);
		NativeObjectRef::n_set_sun(server, player, sun_params);
		NativeObjectRef::n_set_moon(server, player, moon_params);
		NativeObjectRef::n_set_stars(server, player, star_params);
	}
	Server *server = getServer(L);
	NativeObjectRef::n_set_sky(server, player, sky_params);
...
int ObjectRef::l_native_get_sky(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	SkyboxParams skybox_params = NativeObjectRef::n_get_sky(player);
...
int ObjectRef::l_native_get_sky_color(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	const SkyboxParams skybox_params = NativeObjectRef::n_get_sky_color(player);
...
int ObjectRef::l_native_set_sun(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	bool status = NativeObjectRef::n_set_sun(server, player, sun_params);
	if (status) {
		getServer(L)->setSun(player, sun_params);
...
int ObjectRef::l_native_get_sun(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	const SunParams &sun_params = NativeObjectRef::n_get_sun(player);
...
int ObjectRef::l_native_set_moon(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	bool status = NativeObjectRef::n_set_moon(server, player, moon_params);
	if (status)
		lua_pushboolean(L, status);
	return (int)status;
}
...
int ObjectRef::l_native_get_moon(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	const MoonParams &moon_params = NativeObjectRef::n_get_moon(player);
...
int ObjectRef::l_native_set_stars(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	bool status = NativeObjectRef::n_set_stars(server, player, star_params);
...
int ObjectRef::l_native_get_stars(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	const StarParams &star_params = NativeObjectRef::n_get_stars(player);
...
int ObjectRef::l_native_set_clouds(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	luaL_checktype(L, 2, LUA_TTABLE);
	CloudParams cloud_params = NativeObjectRef::n_get_clouds(player);
	...
	Server *server = getServer(L);
	bool status = NativeObjectRef::n_set_clouds(server, player, cloud_params);
	if (status)
		lua_pushboolean(L, status);
...
int ObjectRef::l_native_get_clouds(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	const CloudParams &cloud_params = NativeObjectRef::n_get_clouds(player);
...
int ObjectRef::l_native_override_day_night_ratio(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	Server *server = getServer(L);
	bool status = NativeObjectRef::n_override_day_night_ratio(server, player, do_override, ratio);
...
int ObjectRef::l_native_get_day_night_ratio(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	bool status = NativeObjectRef::n_get_day_night_ratio(player, do_override, ratio);
	if (!status)
		return 0;
	else
	{
...
int ObjectRef::l_native_set_minimap_modes(lua_State *L)
{
	NO_MAP_LOCK_REQUIRED;
	ObjectRef *ref = checkobject(L, 1);
	RemotePlayer *player = getplayer(ref);
	...
	while (lua_next(L, 2) != 0) {
		...
		if (lua_istable(L, -1)) {
			...
			if (type == "off")
				...
				mode.type = MINIMAP_TYPE_TEXTURE;
				mode.texture = getstringfield_default(
						L, -1, "texture", "");
				mode.scale = getintfield_default(L, -1, "scale", 1);
			} else {
				warningstream << "Minimap mode of unknown type \""
					      << type.c_str() << "\" ignored.\n"
					      << std::endl;
				ok = false;
			}
			...
			if (ok) {
				mode.label = getstringfield_default(L, -1, "label", "");
				// Size is limited to 512. Performance gets poor if size
				// too large, and segfaults have been experienced.
				mode.size = rangelim(
						getintfield_default(L, -1, "size", 0), 1,
						512);
				modes.push_back(mode);
			}
		}
	...
	NativeObjectRef::n_set_minimap_modes(server, player, modes, selected_mode);
	return 0;
}
